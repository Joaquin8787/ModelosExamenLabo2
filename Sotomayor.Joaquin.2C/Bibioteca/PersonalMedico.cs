...
namespace Bibioteca
{
    public class PersonalMedico : Persona
    {
        ...
        internal override string FichaExtra()
        {
           ...
           // sb.AppendLine(string.Format("¿Finalizo la residencia? {0}", this.esResidente ? "SI" : "NO"));
            sb.AppendLine("ATENCIONES: ");
            foreach(Consulta consulta in this.consulta)
            {
                sb.AppendLine(FichaPersonal(consulta.Paciente)); //ME FALTABA ESTO
            }
            return sb.ToString();
        }
